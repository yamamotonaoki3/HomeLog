...
> **実装状況**：固定費の登録・一覧・編集・削除、および世帯合計対象フラグの保存はF-05として実装済み。毎月の自動計上バッチ（`@Scheduled`）は未実装であり、7-2章に記載の方針に従って別Issueで対応する。割り勘設定（6-3章、`fixed_cost_splits`）はF-04（割り勘・精算管理）が未実装のため対象外とし、F-04実装後に拡張する。
