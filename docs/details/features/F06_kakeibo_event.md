...
トップ画面のイベント別支出サマリーは、「そのイベントに今年いくら使ったか」を確認する目的のため、**当年（1/1〜12/31）内**の支出を対象に集計する（月次の「今月のお金」カードとは対象期間が異なる点に注意）。

| 項目 | 内容 |
| --- | --- |
| 入力 | イベントID・対象年（デフォルトは当年） |
| 処理 | event_id と expense_date が対象年に該当する expenses を集計しSUM(amount)を計算 |
| 出力 | イベント名：当年合計金額 |
