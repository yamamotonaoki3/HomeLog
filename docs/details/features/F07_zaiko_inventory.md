...
| 処理 | S-10の在庫登録モーダルから入力チェック → inventory_items テーブルに保存 → 閾値判定 → 該当すれば shopping_list_items へ自動追加 |
| 出力 | 登録した在庫アイテム |

※ 買う店の選択肢に目的の店がない場合、在庫登録モーダル内の「＋店を登録」ボタンからその場で店舗マスタに追加できる（追加した店は選択状態になる）。
