...
（口座名をクリックすると、その口座の取引履歴モーダルを開く。日付・内容・カテゴリー・金額（±）・取引後残高を日付降順で表示。[F11_kakeibo_account](features/F11_kakeibo_account.md) 4章）
```
...
対応: [F03_kakeibo_expense](features/F03_kakeibo_expense.md) 6-2章, [F13_kakeibo_income](features/F13_kakeibo_income.md) 7-2章
...
│ 交際費（自作）      │カスタム │ [編集][削除]   │
└──────────────────┴────────┴──────────────┘
（デフォルトカテゴリーは編集・削除ボタンを表示しない）

┌─ カテゴリー登録/編集モーダル ────┐
│ カテゴリー名 [___________________]│
│     [ 登録 or 更新 ]  [ キャンセル ]│
└─────────────────────────────────┘
```
...
```mermaid
flowchart TD
    subgraph Screen["S-23 家計簿カテゴリー管理画面"]
        direction TB
        Tabs["支出カテゴリー／収入カテゴリー タブ"]
        AddBtn["「◯◯カテゴリーを登録」ボタン → 登録モーダル"]
        List["カテゴリー一覧テーブル（カテゴリー名・種別・操作。\nデフォルトカテゴリーは操作列の編集・削除ボタンを非表示）"]
        subgraph Modal["カテゴリー登録/編集モーダル"]
            direction TB
            NameInput["カテゴリー名 入力欄（1〜50文字）"]
        end
        DeleteConfirm["削除確認ダイアログ（使用中のカテゴリーはAPIエラーをトーストで表示し一覧に残す）"]
    end
    Tabs --- AddBtn --- Modal
    List --- DeleteConfirm
```
...
| カテゴリー名 | 種別（デフォルト／カスタム） | 操作（デフォルトは非表示） |
| --- | --- | --- |

- S-05（家計簿一覧画面）ツールバーの「カテゴリー管理」リンクから遷移する。
- タブ切り替えは初回マウント時に支出・収入両方のカテゴリーを取得済みのため、再フェッチせず即座に表示が切り替わる。
- デフォルトは全カード・全項目表示。
